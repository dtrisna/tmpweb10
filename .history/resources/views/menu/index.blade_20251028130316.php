...
<body class="bg-gray-100 text-gray-800">
    <div class="max-w-6xl mx-auto py-10 px-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            ...
            @php
                $gambar = strtolower($kopi->nama) . '.jpg';
                $path = public_path('storage/uploads/' . $gambar);
            @endphp
            <div class="bg-white rounded-lg shadow-md overflow-hidden transition hover:shadow-lg">
                @if(file_exists($path))
                    <img src="{{ asset('storage/uploads/' . $gambar) }}" alt="{{ $kopi->nama }}" class="w-full h-48 object-cover">
                @else
